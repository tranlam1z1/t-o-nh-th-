...
import { fillMaskedImage } from '../services/geminiService';
import { cn } from '../lib/utils';

export default function Inpainter({ onBack }: { onBack: () => void }) {
    const { t } = useLanguage();
    ...
    const draw = (e: ReactMouseEvent<HTMLCanvasElement> | ReactTouchEvent<HTMLCanvasElement>) => {
        ...
        if (tool === 'brush') {
            ...
            // Semi-transparent white for the mask overlay
    ...
    const getMaskedImageDataUrl = useCallback(async (): Promise<string | null> => {
        ...
        // 2. "Cut out" the masked area
        tempCtx.globalCompositeOperation = 'destination-out';
        const maskCanvas = canvasRef.current;
        if(maskCanvas) {
            // Draw the mask from the display canvas onto the full-res temp canvas, scaling it up
    ...
    const handleGenerate = async (instructions?: string) => {
        if (!uploadedImage || !maskExists || !submittedPrompt) return;
        ...
        try {
            ...
            const resultUrl = await fillMaskedImage(submittedPrompt, maskedImageDataUrl, instructions);
    ...
    const handleInitialGenerate = () => {
        setSubmittedPrompt(prompt); // Save the prompt for potential regenerations
        setRefinePrompt(''); // Clear refine prompt on new generation
        handleGenerate();
    }
    ...
    const handleDownload = () => {
        ...
        link.download = 'tracquoc-ai-inpainted.png';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    const isGenerateDisabled = !uploadedImage || !prompt || !maskExists || isLoading;
        ...
        <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-8">
                 ...
                 <div ref={containerRef} className="w-full relative rounded-lg overflow-hidden flex items-center justify-center min-h-[50vh] max-h-[70vh]">
                    {uploadedImage ? (
                        ...
                        <label htmlFor="image-upload" className="w-full h-full min-h-[50vh] flex flex-col items-center justify-center text-center text-neutral-500 border-2 border-dashed border-neutral-700 rounded-lg p-8 cursor-pointer hover:border-neutral-500 hover:bg-black/40 transition-colors" onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}>
                            ...
                            <p className="text-lg font-bold text-neutral-400">{t('inpainter.waitingForImage')}</p>
                            <p className="font-bold text-neutral-300">{t('inpainter.dragAndDrop')}</p>
                            <p>{t('inpainter.clickToSelect')}</p>
            ...
            {/* Controls Container */}
             <div className="w-full bg-black/20 backdrop-blur-md border border-neutral-800 rounded-2xl p-6 shadow-lg flex flex-col">
                <h3 className="font-bold text-2xl text-neutral-200 mb-4">{t('inpainter.controls')}</h3>
                <div className={cn(!uploadedImage && "opacity-50 pointer-events-none")}>
                    <div className="flex-shrink-0 bg-neutral-900/70 rounded-lg p-3 flex items-center justify-between gap-4 flex-wrap mb-4">
                        ...
                        <button onClick={clearCanvas} className="p-2 rounded-md bg-red-500/80 hover:bg-red-500 text-white transition-colors text-sm font-semibold flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg> {t('inpainter.clear')}</button>
                    </div>
                    <div>
                        <label htmlFor="prompt" className="block text-sm font-medium text-neutral-300 mb-1">{t('inpainter.prompt')}</label>
                        <textarea
                            id="prompt"
                            value={prompt}
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 text-black font-bold py-3 px-6 rounded-lg bg-neutral-200 hover:bg-white disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105 mt-4"
                    >
                        {isLoading ? t('inpainter.generating') : t('inpainter.generate')}
                    </button>
                </div>
            </div>
        </div>
     ...
     const renderResultView = () => (
         <div className="w-full flex flex-col items-center gap-8">
            <div className="w-full grid md:grid-cols-2 gap-8">
                 ...
                 <div className="flex flex-col w-full">
                     <h3 className="font-bold text-2xl text-neutral-200 mb-4 text-center">{t('inpainter.inpaintedResult')}</h3>
                    <div className="aspect-square w-full bg-black/20 rounded-lg border-2 border-dashed border-neutral-700 flex items-center justify-center text-neutral-500 text-center relative overflow-hidden">
                        ...
                        {error && !isLoading && (
                            <div className="p-4 text-red-400">
                                <p className="font-semibold mb-2">{t('inpainter.inpaintingFailed')}</p>
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center p-4 relative bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-neutral-900 to-black">
            ...
            >
                ...
                <div className="text-center mb-10">
                    ...
                    <p className="text-xl md:text-2xl text-neutral-400 mt-2">{t('inpainter.subtitle')}</p>
                </div>
